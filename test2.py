...
def check_image_safety(image_input):
    """فحص سلامة الصورة سواء كانت عبر URL أو ملف صورة محلي"""
    api_url = "https://api.jigsawstack.com/v1/validate/nsfw"
    headers = {"x-api-key": api_key}

    try:
        if isinstance(image_input, str):  # إذا كان الإدخال هو URL
            image_url = image_input
            response = requests.get(api_url, headers=headers, params={"url": image_url})
        elif isinstance(image_input, str) and image_input.startswith("http"):  # URL لكن لا يعمل كـ string 
            image_url = image_input
        else:
            with open(image_input, 'rb') as img_file:  # إذا كان الإدخال هو مسار الصورة المحلي
