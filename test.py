...
MEDIA_SCAN_GROUP_ID = -1002091669531 
...
daily_reports = {
    ...
    "deleted_content": [],
    "manual_actions": [],
    "videos_checked": [],  # ✅ قائمة جديدة للفيديوهات
    "gifs_checked": []  # ✅ قائمة جديدة للصور المتحركة (GIF)
}
gbt_enabled = False
commands = [
...
def process_media(content, file_extension, message, media_type):
    """معالجة الميديا، إرسالها إلى مجموعة الفحص، وفحصها"""
    try:
        with tempfile.NamedTemporaryFile(delete=False, suffix=file_extension) as temp_file:
            ...
            frames = [Image.fromarray(clip.get_frame(t)) for t in [i * 2 for i in range(3)]]  # استخراج 3 إطارات
            ...
            for frame in frames:
                with tempfile.NamedTemporaryFile(delete=False, suffix=".jpg") as frame_file:
                    ...
                    # إرسال الإطار إلى مجموعة الفحص
                    sent_image = bot.send_photo(MEDIA_SCAN_GROUP_ID, open(frame_file.name, "rb"))

                    # استخراج رابط الصورة من تيليجرام
                    time.sleep(1)
                    file_info = bot.get_file(sent_image.photo[-1].file_id)
                    image_url = f"https://api.telegram.org/file/bot{TOKEN}/{file_info.file_path}"

                    # فحص الصورة باستخدام API
                    result = check_image_safety(image_url)

                    # ✅ حفظ التقرير عن الفيديو أو الصورة المتحركة
                    media_info = (
                        f"👤 <b>العضو:</b> {get_user_mention(message.from_user.id)}\n"
                        f"🎥 <b>النوع:</b> {media_type}\n"
                        f"🔗 <a href='{image_url}'>عرض الإطار</a>\n"
                        f"📊 <b>النتيجة:</b> {'❌ غير لائق' if result == 'nude' else '✅ سليم'}"
                    )

                    if media_type == "فيديو":
                        daily_reports[message.chat.id]["videos_checked"].append(media_info)
                    else:
                        daily_reports[message.chat.id]["gifs_checked"].append(media_info)

                    # حذف الإطار المؤقت
                    os.unlink(frame_file.name)

                    # إذا كانت النتيجة غير لائقة، أوقف الفحص
                    if result == "nude":
...
def ban_user(message):
    ...
    if not is_user_admin(bot, chat_id, user_id):
        bot.reply_to(message, (
            f"❌ <b>عذرًا!</b>\n"
            f"هذا الأمر مخصص <b>للمشرفين فقط</b>.\n"
            f"⚠️ لا تقم بذلك مرة أخرى، هذا أمر خطير!"
        ), parse_mode="HTML")
        return
    ...
    if not target_id:
        bot.reply_to(message, (
            f"ℹ️ <b>كيفية استخدام الأمر:</b>\n"
            f"1️⃣ بالرد على رسالة العضو: <code>/ban</code>\n"
            f"2️⃣ باستخدام الأيدي: <code>/ban 12345</code>"
        ), parse_mode="HTML")
        return
    ...
    try:
        ...
        bot.send_message(chat_id, (
            f"🚫 <b>تم حظر العضو!</b>\n"
            f"👤 العضو: {get_user_mention(target_id)}\n"
            f"⚠️ لن يتمكن من العودة إلى المجموعة."
        ), parse_mode="HTML")

    except Exception as e:
        bot.reply_to(message, f"❌ <b>حدث خطأ أثناء محاولة حظر العضو:</b> <code>{e}</code>", parse_mode="HTML")
...
def unban_user(message):
    ...
    if not target_id:
        bot.reply_to(message, (
            f"ℹ️ <b>كيفية استخدام الأمر:</b>\n"
            f"1️⃣ بالرد على رسالة العضو: <code>/unban</code>\n"
            f"2️⃣ باستخدام الأيدي: <code>/unban 12345</code>"
        ), parse_mode="HTML")
        return
    ...
    try:
        ...
        bot.send_message(chat_id, (
            f"✅ <b>تم إلغاء حظر العضو!</b>\n"
            f"👤 العضو: {get_user_mention(target_id)}\n"
            f"🎉 الآن يمكنه الانضمام إلى المجموعة مرة أخرى!"
        ), parse_mode="HTML")
    except Exception as e:
        bot.reply_to(message, f"❌ <b>حدث خطأ أثناء محاولة إلغاء الحظر:</b> <code>{e}</code>", parse_mode="HTML")
...
def mute_user(message):
    ...
    if not is_user_admin(bot, chat_id, user_id):
        bot.reply_to(message, (
            f"❌ <b>عذرًا!</b>\n"
            f"هذا الأمر مخصص <b>للمشرفين فقط</b>.\n"
            f"⚠️ لا تحاول مجددًا، هذا أمر خطير!"
        ), parse_mode="HTML")
        return
    ...
    if not target_id:
        bot.reply_to(message, (
            f"ℹ️ <b>كيفية استخدام الأمر:</b>\n"
            f"1️⃣ بالرد على رسالة العضو: <code>/mute</code>\n"
            f"2️⃣ باستخدام الأيدي: <code>/mute 12345</code>\n"
            f"3️⃣ لتقييد مؤقت: <code>/mute 12345 30</code> (30 دقيقة مثال)"
        ), parse_mode="HTML")
        return
    ...
    if message.reply_to_message:
        if len(command_parts) > 1:
            try:
                ...
                bot.reply_to(message, "⚠️ <b>خطأ!</b>\nالمدة الزمنية يجب أن تكون <b>رقمًا صحيحًا</b>.", parse_mode="HTML")
                return
    else:
    ...
    if mute_duration:
        ...
        bot.send_message(chat_id, (
            ...
            f"⌛ بعد انتهاء الوقت، سيعود إلى الدردشة."
        ), parse_mode="HTML")
    else:
        bot.restrict_chat_member(chat_id, target_id, can_send_messages=False)
        bot.send_message(chat_id, (
            f"🔇 <b>تم تقييد العضو بشكل دائم!</b>\n"
            f"👤 العضو: {get_user_mention(target_id)}"
        ), parse_mode="HTML")
...
def unmute_user(message):
    ...
    if not target_id:
        bot.reply_to(message, (
            f"ℹ️ <b>كيفية استخدام الأمر:</b>\n"
            f"1️⃣ بالرد على رسالة العضو: <code>/unmute</code>\n"
            f"2️⃣ باستخدام الأيدي: <code>/unmute 12345</code>"
        ), parse_mode="HTML")
        return
    ...
    try:
        ...
        bot.send_message(chat_id, (
            f"✅ <b>تم إلغاء تقييد العضو!</b>\n"
            f"👤 العضو: {get_user_mention(target_id)}\n"
            f"🎉 الآن يمكنه التحدث بحرية مرة أخرى!"
        ), parse_mode="HTML")
    except Exception as e:
        bot.reply_to(message, f"❌ <b>حدث خطأ أثناء محاولة إلغاء التقييد:</b> <code>{e}</code>", parse_mode="HTML")
...
def handle_photo(message):
    ...
    if res == 'nude':
        ...
        warning_message = (
            f"🚫 <b>لا ترسل صور غير لائقة يا {get_user_mention(message.from_user.id)}!</b>\n"
            f"🫵 <b>{get_user_mention(message.from_user.id)}</b>، <b>هذا تنبيه لك!</b>\n"
...
def handle_sticker(message):
    ...
    if message.sticker.thumb:
        ...
        if res == 'nude':
            ...
            warning_message = (
                f"🚫 <b>لا ترسل ملصقات غير لائقة يا {get_user_mention(message.from_user.id)}!</b>\n"
                f"🫵 <b>{get_user_mention(message.from_user.id)}</b>، <b>هذا تنبيه لك!</b>\n"
                "<b>🤖 البوت يراقب ويمنع المحتوى غير الملائم 🛂</b>"
            )
        ...
        if sticker_links:
            for link in sticker_links:
                ...
                if res == 'nude':
                    bot.delete_message(message.chat.id, message.message_id)
                    warning_message = (
                        f"🚫 <b>لا ترسل رموز غير لائقة يا {get_user_mention(message.from_user.id)}!</b>\n"
                        f"🫵 <b>{get_user_mention(message.from_user.id)}</b>، <b>هذا تنبيه لك!</b>\n"
                        "<b>🤖 البوت يراقب ويمنع المحتوى غير الملائم 🛂</b>"
                    )
                    bot.send_message(message.chat.id, warning_message, parse_mode="HTML")
...
def get_premium_sticker_info(custom_emoji_ids):
    ...
    try:
        ...
        return []
@bot.edited_message_handler(content_types=['video', 'animation'])
def handle_edited_media(message):
    """التعامل مع الفيديوهات والصور المتحركة المعدلة"""
    if message.video:
...
def handle_edited_custom_emoji_message(message):
    ...
    user_id = message.from_user.id
    chat_id = message.chat.id
    content_type = "رمز تعبيري"
    user_violations = {}  # متغير لتخزين عدد المخالفات لكل مستخدم، استبدله بنظامك الحالي

    user_name = get_user_mention(user_id)
                ...
                for link in sticker_links:
                    ...
                    if res == 'nude':
                        ...
                        warning_msg = (
                            ...
                            f"عدد المخالفات: {user_violations.get(user_id, 0) + 1}/10"
                        )
...
def send_group_report(group_id):
    """إرسال التقرير اليومي إلى المجموعة"""
    if group_id in daily_reports and any(daily_reports[group_id].values()):
        report = daily_reports[group_id]
        report_chat_id = activated_groups.get(group_id, group_id)
        ...
        msg += f"🎥 الفيديوهات المفحوصة:\n" + ("\n".join(report["videos_checked"]) if report["videos_checked"] else "لا يوجد") + "\n\n"
        msg += f"🖼 الصور المتحركة المفحوصة:\n" + ("\n".join(report["gifs_checked"]) if report["gifs_checked"] else "لا يوجد")

        bot.send_message(report_chat_id, msg, parse_mode="HTML")

    else:
        bot.send_message(group_id, "📢 لا يوجد سجل للمخالفات اليوم.", parse_mode="HTML")
